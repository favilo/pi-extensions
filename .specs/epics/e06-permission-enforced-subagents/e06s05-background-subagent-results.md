...
A child runs without blocking the foreground agent, exposes a bounded observable event stream, signals completion through steering or a new idle-parent turn, returns output only through explicit retrieval, and leaves no active work after shutdown.
